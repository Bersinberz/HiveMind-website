...
exports.seedDomainsAndTechnologies = void 0;
...
const seedDomainsAndTechnologies = async () => {
    try {
        ...
        for (const data of DOMAINS_SEED_DATA) {
            ...
            for (const techName of data.technologies) {
                ...
                const techSlug = (0, slugify_1.slugify)(normalizedTech);
                // Case-insensitive technology lookups
                let tech = await Technology_1.default.findOne({
                    name: { $regex: new RegExp(`^${escapeRegex(normalizedTech)}$`, "i") }
                });
                if (!tech) {
                    tech = await Technology_1.default.create({
                        name: normalizedTech,
                        slug: techSlug,
            ...
            const domainSlug = (0, slugify_1.slugify)(normalizedDomain);
            let domain = await Domain_1.default.findOne({
                name: { $regex: new RegExp(`^${escapeRegex(normalizedDomain)}$`, "i") }
            });
            if (!domain) {
                domain = await Domain_1.default.create({
                ...
                for (const tId of techIds) {
                    if (!domain.technologies.includes(tId)) {
                        domain.technologies.push(tId);
                    }
                }
                await domain.save();
            }
            // 3. Associate technologies to this domain
            await Technology_1.default.updateMany({ _id: { $in: techIds } }, { $addToSet: { domains: domain._id } });
        }
...
exports.seedDomainsAndTechnologies = seedDomainsAndTechnologies;
/**
        ...
        for (const project of projects) {
            ...
            if (project.domain && project.domain.length > 0) {
                for (const domName of project.domain) {
                    const dom = await Domain_1.default.findOne({
            ...
            if (project.techStack && project.techStack.length > 0) {
                for (const techName of project.techStack) {
                    const tech = await Technology_1.default.findOne({
